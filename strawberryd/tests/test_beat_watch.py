...
spec = importlib.util.spec_from_file_location("beat_watch", Path(__file__).resolve().parents[2] / "doorways" / "beat_watch.py")
beat_watch = importlib.util.module_from_spec(spec)
spec.loader.exec_module(beat_watch)
