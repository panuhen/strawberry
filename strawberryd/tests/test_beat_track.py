...
spec = importlib.util.spec_from_file_location("beat_track", Path(__file__).resolve().parents[2] / "doorways" / "beat_track.py")
beat_track = importlib.util.module_from_spec(spec)
sys.modules["beat_track"] = beat_track  # dataclasses resolve postponed annotations through sys.modules
spec.loader.exec_module(beat_track)  # type: ignore[union-attr]
