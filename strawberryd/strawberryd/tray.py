...
def child_specs(repo_root: Path, port: int, config: Path | None, widget: bool = True) -> list[Child]:
    """The daemon, the three doorways and the widget, in the order they should come up.

    The doorways are package modules on this same interpreter (jeepney, so no system Python);
    the beat watcher is still a script in the checkout and needs only numpy, which the venv has.
    Step 4 of PACKAGING.md replaces `bin/strawberry widget` with the exported binary.
    """
    python = sys.executable
    url = f"http://127.0.0.1:{port}"
    daemon = [python, "-m", "strawberryd", "--port", str(port)]
    if config:
        daemon += ["--config", str(config)]
    children = [Child("daemon", daemon)]
    for module in ("mpris_watch", "notify_watch"):
        children.append(Child(module, [python, "-m", f"strawberryd.doorways.{module}", "--daemon", url]))
    beat = repo_root / "doorways" / "beat_watch.py"
    if beat.is_file():
        children.append(Child("beat_watch", [python, str(beat), "--daemon", url]))
    launcher = repo_root / "bin" / "strawberry"
    if widget and launcher.is_file():
        children.append(Child("widget", [str(launcher), "widget"]))
    return children
...
class Tray:
    ...
    async def open_settings_file(self) -> None:
        ...
        path = default_path()
        if not path.exists():
            process = await asyncio.create_subprocess_exec(sys.executable, "-m", "strawberryd", "--init-config",
...
def repo_root() -> Path:
    """The checkout this package lives in, while there still is one (step 2 moves the assets)."""
...
async def run_tray(port: int, children: bool = True, widget: bool = True, config: Path | None = None) -> int:
    ...
    if children:
        tray.children = Children(child_specs(repo_root(), port, config, widget=widget),
