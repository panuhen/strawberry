...
`assets/icons/strawberry-<n>.png` are rendered once from Noto Color Emoji by
`assets/icons/render_icons.py` and checked in; at runtime we only have to read them, so
Pillow stays a dev dependency and the daemon's venv stays small.
...
def icons_dir() -> Path:
    """Where the PNGs live: inside the package once installed, in the checkout while developing."""
    for candidate in (Path(__file__).resolve().parent / "assets" / "icons",
