...
- **Type to her** — right-click → *Type to her…* (or press T) opens a glass text field under her; Enter sends the line through the daemon exactly like a spoken sentence (gate, reflexes, Qwen) and she answers on the desktop. `bin/strawberry talk` does the same from a terminal, with the routing shown underneath.
