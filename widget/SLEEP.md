...
Desktop inactivity is queried once per second on a worker thread through `desktop_idle.py`. X11 uses XScreenSaver; GNOME Wayland uses Mutter's idle monitor. Only elapsed idle time is read, never key contents. If the API is unavailable, automatic sleep is disabled rather than mistaking activity in another app for inactivity. Other Wayland desktops need their own idle adapter. The source launcher uses system Python 3 and the desktop's existing X11 libraries; future standalone packaging must include the helper.
