...
box, and scaled down to each size the StatusNotifierItem asks for (WIRING.md §14).
    ...
    python3 assets/icons/render_icons.py [--font PATH] [--out DIR]

Needs Pillow (a dev dependency: `uv run --group dev python assets/icons/render_icons.py`, or
...
def render(font_path: str, sizes=SIZES, out_dir: Path = Path("assets/icons")) -> list[Path]:
