...
Source of truth: `src/strawberry/contract.py` and the constants at the top of `widget/widget.gd`.
...
**The children.** `daemon` (`python -m strawberry.strawberryd`), `mpris_watch`, `notify_watch`, `beat_watch` (`python -m strawberry.doorways.<name>`), all on the tray's own interpreter so an installed tray never reaches into a checkout, and the widget (`python -m strawberry widget`, with `STRAWBERRY_TRAY=1` so the CLI does not start a second daemon or a second set of doorways). The widget child exists only when the package runs from a checkout with `widget/project.godot` (step 4 of PACKAGING.md replaces it with the exported binary); installed on its own, the tray logs that and runs the rest. A child that exits is restarted after a backoff that doubles from 1 s to 30 s and resets once the child has stayed up for 30 s; Quit terminates them all, killing what does not go in 5 s. The tray writes `~/.local/state/strawberry/tray.json` (its own pid, each child's pid, command line and restart count) and `strawberry status` prints it.
...
`strawberry config` creates the file from a commented template (`strawberryd --init-config`) and opens it in `$EDITOR`; `strawberry restart` applies it. `STRAWBERRYD_PORT` still overrides the port for scripts. The widget's own preferences (skin, window position) stay in Godot's `user://widget.cfg` for now.
...
src/strawberry/          the package: contract, events/reactor, brain, speech (Piper), voice (whisper), systemone (the gate), tools (MCP client), actions (reflexes), thinker (Qwen tool loop), ledger (short memory), hub, server
                         + cli.py (`strawberry`), strawberryd.py (`strawberryd`), paths.py (XDG dirs, the checkout), bus.py (jeepney plumbing), client.py (HTTP to the daemon), icons.py (PNG -> ARGB32), tray.py (the StatusNotifierItem and the process that owns her, §14), mpris.py, adapters/
src/strawberry/doorways/ notify_watch.py, mpris_watch.py, beat_watch.py + beat_track.py (`strawberry-doorway <name>`, or python -m strawberry.doorways.<name>)
src/strawberry/assets/icons/  the tray icon PNGs (package data), rendered by scripts/render_icons.py
