...
- **Voices** are Piper `.onnx` files in `~/.local/share/strawberry/voices/`. `bin/strawberry voices` lists them, `bin/strawberry voices en_US-amy-medium` downloads one (catalogue: rhasspy.github.io/piper-samples), `bin/strawberry audition` plays a sample line in each, `strawberryd --say TEXT [--voice V]` writes a wav for one. Installed: `en_GB-jenny_dioco-medium` (default, British), `en_GB-alba-medium` (Scottish), `en_US-amy-medium` (brighter, US), `en_GB-alan-medium` (Scottish male). The persona matches: British English, dry wit, no Scots words (an evening as a Scottish crab was fun but Panu switched back).
