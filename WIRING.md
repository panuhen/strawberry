...
3. restarts the `notify_watch` child alone (`Children.restart_child`: at once, no backoff, not counted as a restart). The watcher reads the mode once, at start, and logs `bodies <mode>`; the tray passes it `--config` when the tray has one, so both read the file that was written.
...
src/strawberry_crab/doorways/ notify_watch.py, mpris_watch.py, beat_watch.py + beat_track.py, smtc_watch.py (Windows) (`strawberry-doorway <name>`, or python -m strawberry_crab.doorways.<name>; `for_system()` says which run where)
